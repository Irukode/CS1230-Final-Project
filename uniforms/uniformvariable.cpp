...
UniformVariable *checkStatics(const QString &str, UniformVariable::Type type) {
    ...
    case UniformVariable::TYPE_MAT4:
        ...
        if (qstrcmp(str, "<view>")) return View::s_view;
        if (qstrcmp(str, "<projection>")) return View::s_projection;
        if (qstrcmp(str, "<mvp>")) return View::s_mvp;
        break;
    case UniformVariable::TYPE_TEXCUBE:
        if (qstrcmp(str, "<skybox>")) return View::s_skybox;
        break;
    case UniformVariable::TYPE_FLOAT:
        if (qstrcmp(str, "<time>")) return View::s_time;
        break;
    case UniformVariable::TYPE_FLOAT2:
        if (qstrcmp(str, "<size>")) return View::s_size;
    case UniformVariable::TYPE_FLOAT3:
        if (qstrcmp(str, "<mouse>")) return View::s_mouse;
    default:
        return NULL;
    }
    return NULL;
